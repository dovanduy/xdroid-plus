...
public class HookManager {
	...
	public final static String cReadMessage = "ReadMessage";
	public final static String cReadContacts = "ReadContacts";
	public final static String cReadCalllog = "ReadCalllog";
	public final static String cReadLocation = "ReadLocation";
	public final static String cReadSERIAL = "ReadSERIAL";
	public final static String cSendMessage = "SendMessage";
	public final static String cCallPhone = "CallPhone";
	public final static String cPhoneState = "PhoneState";
	public final static String cAudioRecord = "AudioRecord";
	public final static String cWebHistory = "WebHistory";
	public final static String cTakePhoto = "TakePhoto";
	public final static String c2G3GNetwork = "2G3GNetwork";
	public final static String cWifiNetwork = "WifiNetwork";
	// This should correspond with the above definitions
	private static final String cRestrictionNames[] = new String[] { cAccounts,
			cBrowser, cCalendar, cCalling, cClipboard, cContacts, cDictionary,
			cEMail, cIdentification, cInternet, cIPC, cLocation, cMedia,
			cMessages, cNetwork, cNfc, cNotifications, cOverlay, cPhone,
			cSensors, cShell, cStorage, cSystem, cView, cReadMessage,
			cReadContacts, cReadCalllog, cReadLocation, cReadSERIAL,
			cSendMessage, cCallPhone, cPhoneState, cAudioRecord, cWebHistory,
			cTakePhoto, c2G3GNetwork, cWifiNetwork };
	...
	public final static String cSettingTrust = "Trust";
	// Caching
	...
	static {
		...
		for (Hook hook : Meta.get()) {
			...
			if (!listRestriction.contains(restrictionName))
				Util.log(null, Log.WARN, "Not found restriction="
						+ restrictionName);
			...
			if (permissions != null)
				for (String perm : permissions)
					if (!perm.equals("")) {
						String aPermission = (perm.contains(".") ? perm
	...
	public static boolean getSettingBool(int uid, String name,
			boolean defaultValue, boolean useCache) {
		return Boolean.parseBoolean(getSetting(uid, name,
				Boolean.toString(defaultValue), useCache));
	}

	public static boolean getSettingBool(int uid, String type, String name,
			boolean defaultValue, boolean useCache) {
		return Boolean.parseBoolean(getSetting(uid, type, name,
				Boolean.toString(defaultValue), useCache));
	}
	...
	public static String getSetting(int uid, String type, String name,
			String defaultValue, boolean useCache) {
		...
		Util.log(null, Log.INFO, "1 Check cache" + cached);
		// Get settings
		if (!cached)
			try {
				Util.log(null, Log.INFO,
						"2 MonitorService.getSetting(uid,type,name)(" + uid
								+ "," + type + "," + name + ")");
				value = MonitorService.getSetting(new PSetting(Math.abs(uid),
						type, name, null)).value;
				Util.log(null, Log.INFO, "2 return" + value);
				if (value == null)
					if (uid > 0)
						value = MonitorService.getSetting(new PSetting(0, type,
		...
		if (!willExpire && !cSettingLog.equals(name))
			Util.log(null, Log.INFO, String.format(
					"get setting uid=%d %s/%s=%s%s %d ms", uid, type, name,
					value, (cached ? " (cached)" : ""), ms));
	...
	public static List<PSetting> getSettingList(int uid) {
		...
		try {
			return MonitorService.getClient().getSettingList(uid);
		} catch (Throwable ex) {
	...
	public static boolean getRestriction(final XHook hook, int uid,
			String restrictionName, String methodName, String secret) {
		return getRestrictionExtra(hook, uid, restrictionName, methodName,
				null, secret);
	}

	public static PRestriction getRestrictionEx(int uid,
			String restrictionName, String methodName) {
		PRestriction query = new PRestriction(uid, restrictionName, methodName,
				false);
		try {
			return MonitorService.getRestriction(query, false, "");
	...
	public static TreeMap<String, String> getRestrictions(Context context) {
		...
		TreeMap<String, String> tmRestriction = new TreeMap<String, String>(
				collator);
		for (String restrictionName : getRestrictions()) {
			...
			tmRestriction.put(
					stringId == 0 ? restrictionName : context
							.getString(stringId), restrictionName);
		}
	...
	public static boolean getRestrictionExtra(final XHook hook, int uid,
			String restrictionName, String methodName, String extra,
			String secret) {
		long start = System.currentTimeMillis();
		// Log.d("XMonitor", "getRestrictionExtra(uid-" + uid
//				+ ",restrictionName-" + restrictionName + ",methodName-"
//				+ methodName + ")");
		PRestriction result = new PRestriction(uid, restrictionName,
...
//		Log.d("XMonitor", "check cache-" + cached);
		// Get restriction
		if (!cached)
			try {
//				Log.d("XMonitor", "MonitorService.getRestriction(" + uid + ","
//						+ restrictionName + "," + methodName + ")");
				PRestriction query = new PRestriction(uid, restrictionName,
						methodName, false);
				query.extra = extra;
//				XposedBridge
//						.log("MonitorService.getRestriction(query, true,secret)...");
				result.restricted = MonitorService.getRestriction(query, true,
						secret).restricted;
//				Log.d("XMonitor", "result.restricted-" + result.restricted);
				// Add to cache
				if (result.time >= 0) {
					key.restricted = result.restricted;
					if (result.time > 0) {
						key.setExpiry(result.time);
						Util.log(null, Log.WARN, "Caching " + result
								+ " until " + new Date(result.time));
					}
					synchronized (mRestrictionCache) {
						if (mRestrictionCache.containsKey(key))
		...
		// Result
		long ms = System.currentTimeMillis() - start;
		Util.log(hook, Log.INFO, String.format("get client %s%s %d ms", result,
				(cached ? " (cached)" : ""), ms));

		return result.restricted;
	}

	public static void setRestriction(int uid, String restrictionName,
			String methodName, boolean restricted, boolean asked) {
		checkCaller();
		...
		if (restrictionName == null)
			listRestriction.addAll(HookManager.getRestrictions());
		else
		...
		for (String rRestrictionName : listRestriction)
			listPRestriction.add(new PRestriction(uid, rRestrictionName,
					methodName, restricted, asked));
			...
			if (!getSettingBool(0, cSettingDangerous, false, false))
				for (String rRestrictionName : listRestriction)
					for (Hook md : getHooks(rRestrictionName))
						if (md.isDangerous())
							listPRestriction
									.add(new PRestriction(uid,
											rRestrictionName, md.getName(),
											false, true));
		...
		setSetting(uid, cSettingModifyTime,
				Long.toString(System.currentTimeMillis()));
	}

	public static List<PRestriction> getRestrictionList(int uid,
			String restrictionName) {
		checkCaller();

		try {
			return MonitorService.getClient().getRestrictionList(
					new PRestriction(uid, restrictionName, null, false));
		} catch (Throwable ex) {
	...
	public static void deleteRestrictions(int uid, String restrictionName,
			boolean deleteWhitelists) {
		checkCaller();

		try {
			// Delete restrictions
			MonitorService.getClient().deleteRestrictions(uid,
	...
	public static void setSetting(int uid, String type, String name,
			String value) {
		checkCaller();

		try {
			MonitorService.getClient().setSetting(
					new PSetting(uid, type, name, value));
		} catch (Throwable ex) {
			Util.bug(null, ex);
		}
	}
	...
	public static void checkCaller() {
		if (MonitorService.isRegistered()) {
	...
	public static void setRestrictionList(List<PRestriction> listRestriction) {
		...
		if (listRestriction.size() > 0)
			try {
				MonitorService.getClient().setRestrictionList(listRestriction);
			} catch (Throwable ex) {
	...
	public static Object getDefacedProp(int uid, String name) {
		...
		if (name.equals("SERIAL") || name.equals("%serialno")) {
			...
			return (cValueRandom.equals(value) ? getRandomProp("SERIAL")
					: value);
		}
		...
		// Phone
		if (name.equals("PhoneNumber") || name.equals("getLine1AlphaTag")
				|| name.equals("getLine1Number") || name.equals("getMsisdn")
				|| name.equals("getVoiceMailAlphaTag")
				|| name.equals("getVoiceMailNumber")) {
		...
		if (name.equals("ANDROID_ID")) {
			...
			return (cValueRandom.equals(value) ? getRandomProp("ANDROID_ID")
					: value);
		}
		...
		if (name.equals("getSimOperator")
				|| name.equals("gsm.sim.operator.numeric"))
			// MCC+MNC: test network
			return getSetting(uid, cSettingMcc, "001", true)
					+ getSetting(uid, cSettingMnc, "01", true);
		if (name.equals("getSimOperatorName")
				|| name.equals("gsm.sim.operator.alpha"))
			return getSetting(uid, cSettingOperator, cDeface, true);

		if (name.equals("getSimSerialNumber")
				|| name.equals("getIccSerialNumber"))
		...
		if (name.equals("getSubscriberId")) { // IMSI for a GSM phone
			...
			return (cValueRandom.equals(value) ? getRandomProp("SubscriberId")
					: value);
		}
		...
		if (name.equals("AdvertisingId")) {
			String adid = getSetting(uid, cSettingAdId,
					"DEFACE00-0000-0000-0000-000000000000", true);
		...
		if (name.equals("CID"))
			try {
				return Integer
						.parseInt(getSetting(uid, cSettingCid, "0", true)) & 0xFFFF;
			} catch (Throwable ignored) {
				return -1;
			}
		...
		if (name.equals("LAC"))
			try {
				return Integer
						.parseInt(getSetting(uid, cSettingLac, "0", true)) & 0xFFFF;
			} catch (Throwable ignored) {
				return -1;
			}
	...
	public static String getRandomProp(String name) {
		...
		if (name.equals("ISO3166")) {
			...
			String country = Character.toString(letters.charAt(r
					.nextInt(letters.length())))
					+ Character.toString(letters.charAt(r.nextInt(letters
							.length())));
			return country;
		}
	...
	public static Location getDefacedLocation(int uid, Location location) {
		...
		location.setLatitude(Float.parseFloat(sLat)
				+ (Math.random() * 2.0 - 1.0) * location.getAccuracy() * 9e-6);
		location.setLongitude(Float.parseFloat(sLon)
				+ (Math.random() * 2.0 - 1.0) * location.getAccuracy() * 9e-6);
		location.setAltitude(Float.parseFloat(sAlt)
				+ (Math.random() * 2.0 - 1.0) * location.getAccuracy());
		...
		try {
			for (String restriction : listRestriction) {
				for (String method : mRestart.get(restriction))
					listRestartRestriction.add(getRestrictionEx(uid,
	...
	public static long getUsage(int uid, String restrictionName,
			String methodName) {
		checkCaller();
		...
		try {
			...
			if (restrictionName == null)
				for (String sRestrictionName : getRestrictions())
					listRestriction.add(new PRestriction(uid, sRestrictionName,
							methodName, false));
			else
				listRestriction.add(new PRestriction(uid, restrictionName,
						methodName, false));
			return MonitorService.getClient().getUsage(listRestriction);
		} catch (Throwable ex) {
			Util.bug(null, ex);
			return 0;
		}
	...
	public static boolean hasPermission(Context context,
			ApplicationInfoEx appInfo, String restrictionName) {
		int uid = appInfo.getUid();
		synchronized (mPermissionRestrictionCache) {
			if (mPermissionRestrictionCache.get(uid) == null)
				mPermissionRestrictionCache.append(uid,
						new HashMap<String, Boolean>());
			if (!mPermissionRestrictionCache.get(uid).containsKey(
					restrictionName)) {
				boolean permission = hasPermission(context,
						appInfo.getPackageName(),
						getPermissions(restrictionName));
				mPermissionRestrictionCache.get(uid).put(restrictionName,
						permission);
			}
	...
	public static boolean hasPermission(Context context,
			ApplicationInfoEx appInfo, Hook md) {
		int uid = appInfo.getUid();
		synchronized (mPermissionHookCache) {
			...
			if (!mPermissionHookCache.get(uid).containsKey(md)) {
				...
				List<String> listPermission = (md.getPermissions() == null ? null
						: Arrays.asList(md.getPermissions()));
				boolean permission = hasPermission(context,
						appInfo.getPackageName(), listPermission);
	...
	private static boolean hasPermission(Context context,
			List<String> listPackage, List<String> listPermission) {
		try {
			if (listPermission == null || listPermission.size() == 0
					|| listPermission.contains(""))
				return true;
				...
				if (pInfo != null && pInfo.requestedPermissions != null)
					for (String rPermission : pInfo.requestedPermissions)
						for (String permission : listPermission)
							if (rPermission.toLowerCase().contains(
									permission.toLowerCase())) {
								String aPermission = "android.permission."
										+ permission;
	...
	public static void deleteSettings(int uid) {
		...
		try {
			MonitorService.getClient().deleteSettings(uid);
		} catch (Throwable ex) {
			Util.bug(null, ex);
		}
	}
	...
	public static void deleteUsage(int uid) {
		...
		try {
			MonitorService.getClient().deleteUsage(uid);
		} catch (Throwable ex) {
			Util.bug(null, ex);
		}
	}
	...
	public static void applyTemplate(int uid, String restrictionName,
		...
		// Check on-demand
		boolean ondemand = getSettingBool(0, HookManager.cSettingOnDemand,
				true, false);
		if (ondemand)
			ondemand = getSettingBool(-uid, HookManager.cSettingOnDemand,
					false, false);
		...
		for (String rRestrictionName : listRestriction) {
			// Parent
			String parentValue = getSetting(0, Meta.cTypeTemplate,
					rRestrictionName, Boolean.toString(!ondemand) + "+ask",
					false);
			boolean parentRestricted = parentValue.contains("true");
			boolean parentAsked = (!ondemand || parentValue.contains("asked"));
			listPRestriction.add(new PRestriction(uid, rRestrictionName, null,
					parentRestricted, parentAsked));

			// Childs
			if (methods)
				for (Hook hook : getHooks(rRestrictionName)) {
					String settingName = rRestrictionName + "."
							+ hook.getName();
					String value = getSetting(
							0,
							Meta.cTypeTemplate,
							settingName,
							Boolean.toString(parentRestricted
									&& (hook.isDangerous() ? dangerous : true))
									+ (parentAsked ? "+asked" : "+ask"), false);
					boolean restricted = value.contains("true");
					boolean asked = value.contains("asked");
					listPRestriction.add(new PRestriction(uid,
							rRestrictionName, hook.getName(), parentRestricted
	...
	public static void setSettingList(List<PSetting> listSetting) {
		...
		if (listSetting.size() > 0)
			try {
				MonitorService.getClient().setSettingList(listSetting);
			} catch (Throwable ex) {
	...
	public static List<PRestriction> getUsageList(Context context, int uid,
		...
		try {
			listUsage.addAll(MonitorService.getClient().getUsageList(uid,
					restrictionName == null ? "" : restrictionName));
		} catch (Throwable ex) {
	...
	public static class ParcelableRestrictionCompare implements
			Comparator<PRestriction> {
		@Override
	...
	public static void clear() {
		...
		try {
			MonitorService.getClient().clear();
