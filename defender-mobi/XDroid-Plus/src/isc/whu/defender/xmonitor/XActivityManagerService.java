...
public class XActivityManagerService extends XHook {
	...
	// @formatter:off
	// 4.2+ public long inputDispatchingTimedOut(int pid, final boolean
	// aboveSystem, String reason)
	// 4.3+ public boolean inputDispatchingTimedOut(final ProcessRecord proc,
	// final ActivityRecord activity, final ActivityRecord parent, final boolean
	// aboveSystem, String reason)
	// 4.0.3+ final void appNotResponding(ProcessRecord app, ActivityRecord
	// activity, ActivityRecord parent, boolean aboveSystem, final String
	// annotation)
	...
	public static List<XHook> getInstances() {
		...
		listHook.add(new XActivityManagerService(
				Methods.inputDispatchingTimedOut,
		...
		listHook.add(new XActivityManagerService(Methods.setLockScreenShown,
				Build.VERSION_CODES.JELLY_BEAN_MR1).optional());
	...
	public static boolean canOnDemand() {
		Log.d("Xmonitor",
				"canOnDemand():mFinishedBooting && !mLockScreen && !mSleeping="
						+ mFinishedBooting + " !" + mLockScreen + " !"
						+ mSleeping);
	...
	protected void before(XParam param) throws Throwable {
		if (mMethod == Methods.inputDispatchingTimedOut) {
			try {
				...
				boolean ondemanding = (mOndemandSemaphore != null && mOndemandSemaphore
						.availablePermits() < 1);
				Util.log(this, Log.WARN, "Foreground ANR uid="
						+ getUidANR(param) + " ondemand=" + ondemanding);
				if (ondemanding)
			...
			try {
				...
				boolean ondemanding = (mOndemandSemaphore != null && mOndemandSemaphore
						.availablePermits() < 1);
				Util.log(this, Log.WARN, "Background ANR uid="
						+ getUidANR(param) + " ondemand=" + ondemanding);
				if (ondemanding)
	...
	private int getUidANR(XParam param) throws IllegalAccessException {
		...
		try {
			...
			if (param.args.length > 0 && param.args[0] != null
					&& param.args[0].getClass().equals(pr)) {
