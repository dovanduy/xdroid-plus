...
public class XLocationManager extends XHook {
	...
	// public void addGeofence(LocationRequest request, Geofence fence,
	...
	// public void requestSingleUpdate(String provider, LocationListener
	// listener, Looper looper)
	// public void requestSingleUpdate(Criteria criteria, LocationListener
	// listener, Looper looper)
	// public void requestSingleUpdate(String provider, PendingIntent intent)
	// public void requestSingleUpdate(Criteria criteria, PendingIntent intent)
	// public boolean sendExtraCommand(String provider, String command, Bundle
	// extras)
	...
	private enum Methods {
		addGeofence, addNmeaListener, addProximityAlert, getGpsStatus, getLastKnownLocation, getProviders, isProviderEnabled, removeUpdates, requestLocationUpdates, requestSingleUpdate, sendExtraCommand
	};
	...
	public static List<XHook> getInstances(Object instance) {
		...
		for (Methods loc : Methods.values())
			if (loc == Methods.removeUpdates)
				...
				// listHook.add(new XLocationManager(loc, HookManager.cLocation,
				// className));
				listHook.add(new XLocationManager(loc,
						HookManager.cReadLocation, className));
		return listHook;
	}
	...
	protected void before(XParam param) throws Throwable {
		...
		} else if (mMethod == Methods.addGeofence
		...
		if (mMethod != Methods.addGeofence
			...
			if (mMethod == Methods.isProviderEnabled) {
				...
				if (param.getResult() != null && isRestricted(param)) {
					...
					try {
						Field mSatellites = status.getClass().getDeclaredField(
								"mSatellites");
				...
				if (location != null && isRestricted(param))
					param.setResult(HookManager.getDefacedLocation(
	...
	private void replaceLocationListener(XParam param, int arg)
			throws Throwable {
		if (param.args.length > arg
				&& param.args[arg] != null
				&& LocationListener.class.isAssignableFrom(param.args[arg]
						.getClass())) {
				...
				if (listener != null) {
					...
					synchronized (mListener) {
						...
						if (xListener == null) {
							...
							Util.log(this, Log.WARN,
									"Added count=" + mListener.size() + " uid="
	...
	private void removeLocationListener(XParam param) {
		if (param.args.length > 0
				&& param.args[0] != null
				&& LocationListener.class.isAssignableFrom(param.args[0]
						.getClass())) {
			...
			synchronized (mListener) {
				...
				if (xlistener != null) {
					...
					Util.log(this, Log.WARN,
							"Removed count=" + mListener.size() + " uid="
									+ Binder.getCallingUid());
				}
			}
		} else
	...
	private class XLocationListener implements LocationListener {
		...
		public void onLocationChanged(Location location) {
			mLocationListener.onLocationChanged(location == null ? location
					: HookManager.getDefacedLocation(Binder.getCallingUid(),
							location));
		}
