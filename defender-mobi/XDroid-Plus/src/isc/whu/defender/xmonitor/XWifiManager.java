...
public class XWifiManager extends XHook {
	...
	public static List<XHook> getInstances(Object instance) {
		...
		for (Methods wifi : Methods.values())
			listHook.add(new XWifiManager(wifi, HookManager.cNetwork, className));

		listHook.add(new XWifiManager(Methods.getScanResults, HookManager.cLocation, className));

		// This is to fake "offline", no permission required
		listHook.add(new XWifiManager(Methods.getConnectionInfo, HookManager.cInternet, className));
		return listHook;
	}
	...
	protected void after(XParam param) throws Throwable {
		if (mMethod == Methods.getConfiguredNetworks) {
			...
			if (param.getResult() != null && isRestricted(param)) {
				...
				if (getRestrictionName().equals(HookManager.cInternet)) {
					...
					try {
						...
						fieldBSSID.set(wInfo, HookManager.getDefacedProp(Binder.getCallingUid(), "MAC"));
					} catch (Throwable ex) {
						Util.bug(this, ex);
					}
					...
					try {
						...
						fieldIp.set(wInfo, HookManager.getDefacedProp(Binder.getCallingUid(), "InetAddress"));
					} catch (Throwable ex) {
						Util.bug(this, ex);
					}
					...
					try {
						...
						fieldMAC.set(wInfo, HookManager.getDefacedProp(Binder.getCallingUid(), "MAC"));
					} catch (Throwable ex) {
						Util.bug(this, ex);
					}

					// SSID
					String ssid = (String) HookManager.getDefacedProp(Binder.getCallingUid(), "SSID");
					try {
			...
			if (param.getResult() != null && isRestricted(param)) {
				...
				dInfo.ipAddress = (Integer) HookManager.getDefacedProp(Binder.getCallingUid(), "IPInt");
