...
public class XLocationClient extends XHook {
	...
	public static List<XHook> getInstances() {
		...
		listHook.add(new XLocationClient(Methods.addGeofences, HookManager.cLocation).optional());
		listHook.add(new XLocationClient(Methods.getLastLocation, HookManager.cLocation).optional());
		...
		listHook.add(new XLocationClient(Methods.requestLocationUpdates, HookManager.cLocation).optional());
		return listHook;
	}
	...
	protected void before(XParam param) throws Throwable {
		if (mMethod == Methods.addGeofences) {
			...
			if (isRestricted(param, HookManager.cLocation, "GMS.addGeofences"))
				param.setResult(null);
	...
	protected void after(XParam param) throws Throwable {
		if (mMethod == Methods.addGeofences || mMethod == Methods.removeGeofences) {
			...
			if (location != null && isRestricted(param))
				param.setResult(HookManager.getDefacedLocation(Binder.getCallingUid(), location));
	...
	private class XLocationListener implements LocationListener {
		...
		public void onLocationChanged(Location location) {
			if (location != null)
				location = HookManager.getDefacedLocation(Binder.getCallingUid(), location);
