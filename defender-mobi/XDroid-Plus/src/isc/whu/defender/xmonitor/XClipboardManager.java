...
public class XClipboardManager extends XHook {
	...
	public String getClassName() {
		return (mClassName == null ? "android.content.ClipboardManager"
				: mClassName);
	}

	// @formatter:off

	// public void addPrimaryClipChangedListener(OnPrimaryClipChangedListener
	// what)
	...
	// public void
	// removePrimaryClipChangedListener(ClipboardManager.OnPrimaryClipChangedListener
	// what)
	...
	public static List<XHook> getInstances(Object instance) {
		String className = (instance == null ? null : instance.getClass()
				.getName());
		...
		for (Methods clip : Methods.values())
			if (clip == Methods.removePrimaryClipChangedListener)
				...
				listHook.add(new XClipboardManager(clip,
						HookManager.cClipboard, className));
		return listHook;
	}
	...
	protected void before(XParam param) throws Throwable {
		if (mMethod == Methods.addPrimaryClipChangedListener) {
			...
			if (isRestricted(param, HookManager.cClipboard,
	...
	protected void after(XParam param) throws Throwable {
		if (mMethod == Methods.addPrimaryClipChangedListener
				|| mMethod == Methods.removePrimaryClipChangedListener)
			;

		else if (mMethod == Methods.getPrimaryClip
				|| mMethod == Methods.getPrimaryClipDescription
				|| mMethod == Methods.getText) {
			if (param.getResult() != null && isRestricted(param))
				param.setResult(null);

		} else if (mMethod == Methods.hasPrimaryClip
				|| mMethod == Methods.hasText) {
			if (isRestricted(param))
