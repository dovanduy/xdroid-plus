...
public class XAccountManager extends XHook {
	...
	public static List<XHook> getInstances(Object instance) {
		...
		listHook.add(new XAccountManager(Methods.addOnAccountsUpdatedListener, HookManager.cAccounts, className));
		listHook.add(new XAccountManager(Methods.blockingGetAuthToken, HookManager.cAccounts, className));
		listHook.add(new XAccountManager(Methods.getAccounts, HookManager.cAccounts, className));
		listHook.add(new XAccountManager(Methods.getAccountsByType, HookManager.cAccounts, className));
		listHook.add(new XAccountManager(Methods.getAccountsByTypeForPackage, HookManager.cAccounts, className));
		listHook.add(new XAccountManager(Methods.getAccountsByTypeAndFeatures, HookManager.cAccounts, className));
		listHook.add(new XAccountManager(Methods.getAuthenticatorTypes, HookManager.cAccounts, className));
		listHook.add(new XAccountManager(Methods.getAuthToken, HookManager.cAccounts, className));
		listHook.add(new XAccountManager(Methods.getAuthTokenByFeatures, HookManager.cAccounts, className));
		listHook.add(new XAccountManager(Methods.hasFeatures, HookManager.cAccounts, className));
	...
	private boolean isAccountAllowed(String accountName, String accountType, int uid) {
		try {
			...
			if (HookManager.getSettingBool(uid, Meta.cTypeAccount, sha1, false, true))
				return true;
		} catch (Throwable ex) {
			Util.bug(this, ex);
		}
