...
public class XActivity extends XHook {
	...
	public static List<XHook> getInstances() {
		...
		for (Methods activity : startMethods)
			listHook.add(new XActivity(activity, HookManager.cView, Intent.ACTION_VIEW));

		// Intent send: call
		for (Methods activity : startMethods)
//			listHook.add(new XActivity(activity, HookManager.cCalling, Intent.ACTION_CALL));
			listHook.add(new XActivity(activity, HookManager.cCallPhone, Intent.ACTION_CALL));

		// Intent send: media
		for (Methods activity : startMethods) {
			listHook.add(new XActivity(activity, HookManager.cMedia, MediaStore.ACTION_IMAGE_CAPTURE));
			listHook.add(new XActivity(activity, HookManager.cMedia, MediaStore.ACTION_IMAGE_CAPTURE_SECURE));
			listHook.add(new XActivity(activity, HookManager.cMedia, MediaStore.ACTION_VIDEO_CAPTURE));
		}
		...
		if (mMethod == Methods.getSystemService) {
			if (param.args.length > 0 && param.args[0] != null) {
				...
				if (name != null && instance != null)
					XMonitor.handleGetSystemService(this, name, instance);
			}
		}
	}
}
