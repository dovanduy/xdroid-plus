...
public class ApplicationInfoEx implements Comparable<ApplicationInfoEx> {
			...
			for (String packageName : packages)
				try {
					ApplicationInfo appInfo = pm.getApplicationInfo(
							packageName, 0);
					mMapAppInfo.put(pm.getApplicationLabel(appInfo).toString(),
							appInfo);
	...
	public static List<ApplicationInfoEx> getXApplicationList(Context context,
		...
		for (int app = 0; app < listAppInfo.size(); app++) {
			...
			Util.log(null, Log.INFO, "package=" + appInfo.packageName + " uid="
					+ appInfo.uid);

			ApplicationInfoEx appInfoEx = new ApplicationInfoEx(context,
					appInfo.uid);
	...
	private void getPackageInfo(Context context, String packageName)
	...
	public Drawable getIcon(Context context) {
		if (mIcon == null)
			...
			if (mMapAppInfo.size() > 0)
				mIcon = mMapAppInfo.firstEntry().getValue()
	...
	public boolean hasInternet(Context context) {
		if (!mInternetDetermined) {
			PackageManager pm = context.getPackageManager();
			for (ApplicationInfo appInfo : mMapAppInfo.values())
				if (pm.checkPermission("android.permission.INTERNET",
	...
	public boolean isFrozen(Context context) {
		if (!mFrozenDetermined) {
			...
			for (ApplicationInfo appInfo : mMapAppInfo.values())
				try {
					int setting = pm
							.getApplicationEnabledSetting(appInfo.packageName);
	...
	public int getState(Context context) {
		return Integer.parseInt(HookManager.getSetting(getUid(),
				HookManager.cSettingState, "1", false));
	}
	...
	public long getModificationTime(Context context) {
		return Long.parseLong(HookManager.getSetting(getUid(),
				HookManager.cSettingModifyTime, "0", false));
	}
	...
	public boolean isSystem() {
		...
		for (ApplicationInfo appInfo : mMapAppInfo.values()) {
			...
			mSystem = mSystem
					|| appInfo.packageName.equals(this.getClass().getPackage()
							.getName());
			mSystem = mSystem
					|| appInfo.packageName.equals(this.getClass().getPackage()
							.getName()
	...
	public boolean isShared() {
		for (ApplicationInfo appInfo : mMapAppInfo.values())
			if (HookManager.isShared(appInfo.uid))
				return true;
		return false;
	}
	...
	public boolean isIsolated() {
		for (ApplicationInfo appInfo : mMapAppInfo.values())
			if (HookManager.isIsolated(appInfo.uid))
				return true;
		return false;
	}
	...
	public String toString() {
		...
		return String.format("%d %s", getUid(),
