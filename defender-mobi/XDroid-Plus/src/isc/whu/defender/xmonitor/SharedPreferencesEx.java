...
public final class SharedPreferencesEx implements SharedPreferences {
	...
	public SharedPreferencesEx(String packageName, String prefFileName) {
		mFile = new File(Util.getUserDataDirectory(Process.myUid())
				+ File.pathSeparator + "shared_prefs" + File.pathSeparator
				+ prefFileName + ".xml");
	...
	private void loadFromDiskLocked() {
		int tries = 0;
		while (++tries <= cTryMaxCount && !mLoaded
				&& (mFile.exists() || mBackupFile.exists())) {
			// Log retry
			if (tries > 1)
				Util.log(
						null,
						Log.WARN,
						"Load " + mFile + " try=" + tries + " exists="
								+ mFile.exists() + " readable="
								+ mFile.canRead() + " backup="
								+ mBackupFile.exists());
			...
			if (mFile.exists() && mFile.canRead() && !mBackupFile.exists()) {
				...
				try {
					...
					Util.log(null, Log.WARN, "Error reading " + mFile + ": "
							+ ex);
				} finally {
					if (str != null) {
						try {
							...
							Util.log(null, Log.WARN, "Error closing " + mFile
									+ ": " + ex);
						}
					}
				}
